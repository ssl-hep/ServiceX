...
class S3Store(object_storage_manager.ObjectStore):
    ...
    def delete_bucket(self, bucket: str) -> bool:
        ...
        if not self.__s3_client.bucket_exists(bucket):
            return True
        delete_objects = map(lambda x: DeleteObject(x.object_name),
                             self.__s3_client.list_objects(bucket))
        errors = 0
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.__threads) as executor:
            ...
            for bucket in bucket_list:
                bucket_age = (datetime.datetime.now(
                    datetime.timezone.utc) - bucket.last_modified).days
            ...
            futures = {
                executor.submit(lambda x: self.delete_bucket(x), bucket[0]): (bucket[0], bucket[1])
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    self.logger.exception(
                        "Received exception while deleting %s due to age", bucket_info[0])
