...
def upgrade():
    ...
    op.alter_column("requests", "files", existing_type=sa.INTEGER(), nullable=False)
...
def downgrade():
    ...
    op.create_index(
        "ix_transform_result_transform_status",
        "transform_result",
        ["transform_status"],
        unique=False,
    )
    op.create_index(
        "ix_transform_result_request_id",
        "transform_result",
        ["request_id"],
        unique=False,
    )
    op.alter_column("requests", "files", existing_type=sa.INTEGER(), nullable=True)
    op.drop_constraint(None, "files", type_="foreignkey")
    op.create_foreign_key(
        "files_dataset_id_fkey",
        "files",
        "datasets",
        ["dataset_id"],
        ["id"],
