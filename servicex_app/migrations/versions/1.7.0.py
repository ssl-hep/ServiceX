...
def upgrade():
    op.add_column(
        "transform_result", sa.Column("created_at", sa.DateTime(), nullable=True)
    )
    ...
    op.create_index(
        "ix_transform_result_created_at", "transform_result", ["created_at"]
    )

    op.add_column(
        "transform_result",
        sa.Column("s3_object_name", sa.String(length=512), nullable=True),
    )

    op.create_index(
        "ix_transform_result_request_id", "transform_result", ["request_id"]
    )
