...
def upgrade():
    ...
    op.add_column(
        "requests", sa.Column("files_completed", sa.Integer(), nullable=False)
    )
    ...
    op.create_index(
        op.f("ix_transform_result_request_id"),
        "transform_result",
        ["request_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_transform_result_transform_status"),
        "transform_result",
        ["transform_status"],
        unique=False,
    )
...
def downgrade():
    ...
    op.drop_index(
        op.f("ix_transform_result_transform_status"), table_name="transform_result"
    )
