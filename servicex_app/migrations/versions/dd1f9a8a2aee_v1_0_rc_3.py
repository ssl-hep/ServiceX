...
def upgrade():
    ...
    op.create_foreign_key(
        "requests_submitted_by_fkey", "requests", "users", ["submitted_by"], ["id"]
    )

    op.add_column(
        "requests", sa.Column("app_version", sa.String(length=64), nullable=True)
    )
    op.add_column(
        "requests", sa.Column("code_gen_image", sa.String(length=256), nullable=True)
    )
