...
def upgrade():
    ...
    op.create_index(op.f("ix_dataset_id"), "files", ["dataset_id"], unique=False)
...
def downgrade():
    ...
    op.execute(
        "alter table requests ALTER COLUMN status TYPE VARCHAR(128) using transformstatus::VARCHAR"
    )  # noqa;
    ...
    op.add_column(
        "transform_result", sa.Column("messages", sa.Integer(), nullable=True)
    )
    op.add_column(
        "requests", sa.Column("workflow_name", sa.String(length=40), nullable=False)
    )
    op.add_column(
        "requests", sa.Column("columns", sa.String(length=1024), nullable=True)
    )
