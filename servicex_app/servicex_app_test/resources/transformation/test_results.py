...
class TestTransformationResults(ResourceTestBase):
    ...
    def mock_transformation_result(self, mocker):
        return mocker.patch('servicex_app.resources.transformation.results.TransformationResult')
    ...
    def test_get_results_with_samples(
        ...
        assert 'results' in data
        assert isinstance(data['results'], list)
        assert len(data['results']) == 3  # We expect 3 results from our sample_results function
    ...
    def test_get_results_with_invalid_later_than_format(
        ...
        assert 'later_than value invalid-datetime-format is not an ISO 8601 compliant datetime' in data['message']