        ...
        lambda selection: (
            dak.zip(
                (
                    {
                        key: (
                            dak.zip(value, depth_limit=(None if len(value) == 1 else 1))
                ...
                depth_limit=(None if len(selection) == 1 else 1),
            )
            if not isinstance(selection, dak.Array)
            else selection
        )
    )(
        (lambda e: {"el_pt_NOSYS": e["el_pt_NOSYS"]})(
            (
                ...
                tree_name if tree_name is not None else "reco",
            )
        )
    ).compute()
